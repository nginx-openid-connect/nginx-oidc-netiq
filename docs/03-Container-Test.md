# How To Locally Test Containerized NGINX Plus OIDC for NetIQ Access Manager Integration

Take the following steps to run NGINX Plus in a Docker container and test OIDC for NetIQ Access Manager integration.
...
- **Run** a Web Browser with `http://nginx.netiq.test:19000`, and enter an `IdP application`'s `client ID`:

  ![](./img/run-browser.png)

- **Login** to click `Sign in` button:

  Enter your name and password which are registered in NetIQ Access Manager.
  > TODO: Replace the following image with NetIQ Access Manager
